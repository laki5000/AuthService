...
export class AuthService {  
  ...
  login(request: LoginDto): Observable<ResultDto<string>> {
    return this.restService.post<ResultDto<string>>(`${UserApiUrlConstant.login}`, request).pipe(
  ...
  register(request: RegisterDto): Observable<ResultDto<string>> {
    return this.restService.post<ResultDto<string>>(`${UserApiUrlConstant.register}`, request).pipe(
      tap((result: ResultDto<string>) => this.loggedInSubject.next(result.result ? true : false))
    );
  }

  logout(): Observable<ResultDto<boolean>> {
    return this.restService.post<ResultDto<boolean>>(`${UserApiUrlConstant.logout}`, null).pipe(
